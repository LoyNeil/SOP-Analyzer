...
const WAIT_FOR_API_AT_STEP = 2

const Loader = ({ onComplete, apiPromise }) => {
  const [currentStep, setCurrentStep] = useState(0)
  const [apiDone, setApiDone]         = useState(false)

  useEffect(() => {
    if (!apiPromise) return
    apiPromise.finally(() => setApiDone(true))
  }, [apiPromise])
  ...
  useEffect(() => {
    ...
    if (currentStep === WAIT_FOR_API_AT_STEP && !apiDone) return
  ...
  useEffect(() => {
    if (apiDone && currentStep === WAIT_FOR_API_AT_STEP) {
